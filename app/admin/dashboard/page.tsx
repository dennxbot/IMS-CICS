...
async function getAdminStats() {
  ...
  try {
    ...
    const weeklyHours = weeklyHoursData?.reduce((sum, record) => sum + (record.total_hours || 0), 0) || 0;
    ...
    for (const student of allStudents || []) {
      const { data: studentHours } = await supabase
        .from('timesheets')
        .select('total_hours')
        .eq('student_id', student.id)
        .eq('timer_status', 0);
      
      const totalRendered = studentHours?.reduce((sum, record) => sum + (record.total_hours || 0), 0) || 0;
